...
fn generateHwConfignNix(allocator: Allocator, config: InitConfigConfiguration, args: InitConfigArgs, virt_type: VirtualizationType) ![]const u8 {
...
fn initConfig(allocator: Allocator, args: InitConfigArgs) !void {
    ...
    _ = flake_nix_file.write(flake_template) catch |err| {
        switch (err) {
            error.AccessDenied => {
